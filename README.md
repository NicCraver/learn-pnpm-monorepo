...
为了便于后续的演示，先在工程根目录下新建 `packages` 目录，并且在 `packages` 目录下创建 `pkg1` 和 `pkg2` 两个工程，分别进到 `pkg1` 和 `pkg2` 两个目录下，执行 `npm init` 命令，初始化两个工程，`package.json` 中的 `name` 字段分别叫做 `@thenic/menorepo1` 和 `@thenic/monorepo2`(*PS：@thenic是提前在npm上创建好的组织，没有的话需要提前创建*)。
...
```ts
// pkg1/src/index.ts
import pkg2 from '@thenic/monorepo2';
...
```bash
$ pnpm add axios --filter @thenic/monorepo1
```
...
```bash
$ pnpm build --filter @thenic/monorepo1
```
...
```bash
$ pnpm install @thenic/monorepo2 -r --filter @thenic/monorepo1
```

此时我们查看 pkg1 的 `package.json`，可以看到 `dependencies` 字段中多了对 `@thenic/monorepo2` 的引用，以 `workspace:` 开头，后面跟着具体的版本号。

```json
{
  "name": "@thenic/monorepo1",
  "version": "1.0.0",
  "dependencies": {
    "@thenic/monorepo2": "workspace:^1.0.0",
    "axios": "^0.27.2"
  }
}
```
...
```json
{
  "build": "pnpm --filter=@thenic/* run build"
}
```
